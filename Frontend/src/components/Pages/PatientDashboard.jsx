...
import { 
  Calendar, Clock, FileText, Phone, Activity, 
  ShieldCheck, HeartPulse, Droplet, ChevronRight, Video, 
  MapPin, BellRing, XCircle, Edit3, CheckCircle2, Eye, Menu
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10 md:mb-12">
            ...
            <div className="flex w-full md:w-auto justify-center gap-3 md:gap-4 mt-4 md:mt-0">
              ...
              <Link to="/appointment" className="w-full md:w-auto justify-center bg-[#dfff4f] hover:bg-white text-[#021814] font-bold font-poppins px-6 py-3 rounded-full transition-all shadow-[0_4px_20px_rgba(223,255,79,0.2)] flex items-center gap-2 hover:-translate-y-1">
                Book Visit <ChevronRight size={18} />
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="lg:col-span-1 bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl md:rounded-3xl p-5 md:p-6 flex flex-row lg:flex-col justify-between items-center lg:items-stretch">
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center w-full lg:mb-4 gap-1">
                <span className="text-[#a8cfc3] font-medium text-sm">Health Score</span>
                <button onClick={() => setIsVitalsModalOpen(true)} className="text-[#dfff4f] hover:text-white text-xs font-bold uppercase tracking-wider w-max">Update</button>
              </div>
              <div className="flex items-end gap-1">
                <span className="text-4xl md:text-5xl font-poppins font-bold text-white leading-none">{vitalsData.score}</span>
            ...
            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl p-5 flex items-center gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 bg-red-500/20 text-red-400 rounded-full flex items-center justify-center"><HeartPulse size={20} /></div>
                <div>
                  <p className="text-[#a8cfc3] text-[10px] md:text-xs font-semibold uppercase tracking-wider">Heart Rate</p>
                  <p className="text-xl md:text-2xl font-bold text-white font-poppins">{vitalsData.heartRate} <span className="text-xs md:text-sm font-normal text-gray-400">bpm</span></p>
                </div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl p-5 flex items-center gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 bg-blue-500/20 text-blue-400 rounded-full flex items-center justify-center"><Droplet size={20} /></div>
                <div>
                  <p className="text-[#a8cfc3] text-[10px] md:text-xs font-semibold uppercase tracking-wider">Blood Pres.</p>
                  <p className="text-xl md:text-2xl font-bold text-white font-poppins">{vitalsData.bpSys}/{vitalsData.bpDia}</p>
                </div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl p-5 flex items-center gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 bg-orange-500/20 text-orange-400 rounded-full flex items-center justify-center"><Activity size={20} /></div>
                <div>
                  <p className="text-[#a8cfc3] text-[10px] md:text-xs font-semibold uppercase tracking-wider">Weight</p>
                  <p className="text-xl md:text-2xl font-bold text-white font-poppins">{vitalsData.weight} <span className="text-xs md:text-sm font-normal text-gray-400">kg</span></p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
              ...
              <div className="flex p-1 bg-gray-100 rounded-xl w-full sm:w-max overflow-hidden">
                <button onClick={() => setActiveTab('upcoming')} className={`flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-semibold transition-all ${activeTab === 'upcoming' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  Upcoming
                </button>
                <button onClick={() => setActiveTab('past')} className={`flex-1 sm:flex-none px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-semibold transition-all ${activeTab === 'past' ? 'bg-white text-[#0F766E] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  Past
                </button>
              </div>
            </div>
            ...
            {loading ? (
              <div className="py-12 text-center text-gray-400 text-sm">Loading your schedule...</div>
            ) : displayApts.length === 0 ? (
              ...
              <div className="space-y-4">
                {displayApts.map((apt) => {
                  ...
                  return (
                    <div key={apt._id} className={`p-4 md:p-5 rounded-xl md:rounded-2xl border transition-all ${isCancelled ? 'bg-gray-50 border-gray-100' : 'bg-white border-gray-200 hover:border-[#0F766E]/30 hover:shadow-md'}`}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        
                        <div className="flex items-center gap-3 md:gap-4">
                          <div className={`w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-full flex items-center justify-center text-lg md:text-xl ${isCancelled ? 'bg-gray-200 grayscale' : 'bg-[#E6F4F1]'}`}>
                            👩‍⚕️
                          </div>
                          <div>
                            <h3 className={`font-bold font-poppins text-base md:text-lg leading-tight ${isCancelled ? 'text-gray-500 line-through' : 'text-[#021814]'}`}>
                              {apt.doctorName}
                            </h3>
                      ...
                      <div className="mt-4 pt-3 md:pt-4 border-t border-gray-100 flex flex-wrap items-center justify-between gap-2">
                        {isCancelled ? (
                          <Badge variant="destructive" className="bg-red-50 text-red-600 hover:bg-red-50 border-none font-bold shadow-none gap-1 text-[10px] md:text-xs">
                            <XCircle size={12}/> Cancelled
                          </Badge>
                        ) : (
                        ...
                        <div className="flex flex-wrap gap-1 md:gap-2">
                          <button onClick={() => openDetails(apt)} className="text-[10px] md:text-xs font-semibold text-gray-600 hover:bg-gray-100 px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1">
                            <Eye size={14}/> View
                          </button>

                          {!isCancelled && activeTab === 'upcoming' && (
                            <>
                              <button onClick={() => openReschedule(apt)} className="text-[10px] md:text-xs font-semibold text-blue-600 hover:bg-blue-50 px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1">
                                <Edit3 size={14}/> Reschedule
                              </button>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent className="rounded-2xl md:rounded-[2rem] w-[90vw] md:w-auto max-w-md">
                                  <AlertDialogHeader>
                                    <AlertDialogTitle className="font-poppins text-lg md:text-xl text-[#021814]">Cancel Appointment?</AlertDialogTitle>
                                    <AlertDialogDescription className="font-inter text-sm">
                                      This will permanently cancel your appointment with <strong>{apt.doctorName}</strong>. 
                                      <br/>Note: Cancellations are not allowed within 24 hours.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
            ...
            <div className="bg-white rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-8">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-2 gap-2 md:gap-3">
                {[
                  ...
                  <button key={idx} className={`p-3 md:p-4 rounded-xl md:rounded-2xl flex flex-col items-center justify-center gap-1.5 md:gap-2 hover:shadow-md transition-all ${item.bg} group`}>
